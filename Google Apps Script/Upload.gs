...
/** ===== Debug settings ===== */
// Set to false in production to reduce GAS execution time and quota usage
const DEBUG_VERBOSE = false;
const DEBUG_MAX_PAYLOAD = 5000;
...
// Cache spreadsheet reference to avoid repeated openById calls
let _ssCache = null;
function getSS(){
...
function logDebug(tag,msg,obj){
  ...
  try{
    const sh = getSS().getSheetByName(SHEETS.DEBUG);
    if(!sh) return;
    if(sh.getLastRow()===0) sh.appendRow(getHeadersForSheet(SHEETS.DEBUG));
    sh.appendRow([
      nowIso(),
      tag,
      msg,
...
// Fix race condition: use LockService to prevent concurrent header writes
function ensureHeader(name){
  ...
  try {
    ...
    // Lock timeout or error - headers likely already exist
  } finally {
    lock.releaseLock();
  }
}
...
function logError(where,message,payload){
  try{
    ...
    getSheet(SHEETS.ERR).appendRow([
      ...
      where,
      message,
      payload?safeJson(payload).slice(0,5000):''
    ]);
  }catch(e){}
}
...
function doPost(e){
  try{
    ...
    logDebug("doPost","Incoming",{action});
    switch(action){
  ...
  try{
    ...
    const indicators=readTable(SHEETS.CONFIG_IND).rows.filter(r=>{
      return String(r.Scope).trim().toLowerCase()==='individual'
        && (String(r.RoomCode).trim()===room||String(r.RoomCode).trim()==='*'||String(r.RoomCode).trim()==='')
        && (String(r.SlotCode).trim()===slot||String(r.SlotCode).trim()==='*'||String(r.SlotCode).trim()==='');
    });
    ...
    staffIds.forEach(staffId=>{
      ...
      (items||[]).forEach(it=>{
        ...
        rowsForm.push([
          ...
          by,'Individual',staffId,staffName,(h.presentStaffIds||[]).map(String).join(','),
...
/**
 * Build weight index: Map<"Category|Indicator" -> [{room,slot,weight}]>
 * Called once per summary computation instead of O(N*M) linear scans
 */
function buildWeightIndex(configRows){
  const idx = {};
  configRows.forEach(row => {
    const c = String(row.Category||'').trim();
    const i = String(row.Indicator||'').trim();
    const key = c + '|' + i;
    if(!idx[key]) idx[key] = [];
    idx[key].push({
      room: String(row.RoomCode||'').trim(),
      slot: String(row.SlotCode||'').trim(),
      weight: (row.Weight === '' || row.Weight === null || row.Weight === undefined) ? 1 : (Math.max(0, Number(row.Weight)) || 1)
    });
  });
  return idx;
}

function getWeightFromIndex(weightIndex, roomCode, slotCode, category, indicator){
  const key = String(category||'').trim() + '|' + String(indicator||'').trim();
  const entries = weightIndex[key];
  if(!entries) return 1;
  const r = String(roomCode||'').trim();
  const s = String(slotCode||'').trim();
  for(let k = 0; k < entries.length; k++){
    const e = entries[k];
    if(e.room !== '' && e.room !== '*' && e.room !== r) continue;
    if(e.slot !== '' && e.slot !== '*' && e.slot !== s) continue;
    return e.weight;
  }
  return 1;
}
...
function recomputeSummary(level){
  ...
  const weightIndex = buildWeightIndex(config);
  ...
  for(const k in grouped){
    ...
    g.rows.forEach(row=>{
      const weight = getWeightFromIndex(weightIndex, row.RoomCode, row.SlotCode, row.Category, row.Indicator);
